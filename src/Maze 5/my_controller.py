...
class RosbotExplorer:
    def __init__(self):
        ...
        self.blue_plan_failure_count = 0
        self.yellow_plan_failure_count = 0
        ...
        self.YELLOW_LOCK_SAMPLES = 3
        self.YELLOW_LOCK_MAX_SPREAD_M = 0.45
        ...
        # Supervisor-free green handling.
        # Do NOT project every visible green blob into the map. In Maze 5 the
        # green floor can be visible far ahead, and depth/centroid projection
        # produces large false green blobs. We only mark green when it is low in
        # the camera image / close to the robot, then mark it as normal black
        # OCC cells exactly like the previous map style.
        # Conservative green mapping. Earlier versions marked a wide black fan
        # whenever green was visible. That polluted the map with black cells in
        # places where no green floor existed. Now green is marked only when it
        # is close in the lower camera view, and the marked patch is compact.
        self.GREEN_MAP_MIN_COVERAGE = 0.012      # ignore tiny green speckles
        self.GREEN_NEAR_BOTTOM_FRAC = 0.82       # green centroid must be low in ROI
        self.GREEN_NEAR_IMAGE_Y_FRAC = 0.70      # or low in full image
        self.GREEN_APPROACH_COVERAGE = 0.30      # coverage alone almost never triggers
        self.GREEN_MARK_AHEAD_M = 0.42
        self.GREEN_MARK_START_M = 0.18
        self.GREEN_MARK_CONE_DEG = 14.0
        self.GREEN_MARK_CLOSE_CONE_DEG = 20.0
        self.GREEN_KEEP_ROBOT_CLEAR_RADIUS_M = 0.18
        self.GREEN_PIXEL_MAX_DISTANCE_M = 0.85
        self.GREEN_PIXEL_SAMPLE_STEP = 6
        self.GREEN_PIXEL_INFLATE_CELLS = 1
        self.GREEN_MAX_NEW_CELLS_PER_MARK = 90
        self.GREEN_MARK_COOLDOWN_S = 1.2
        self.green_obstacle_cells = set()
        # Green scan made the Supervisor-free robot spend long periods spinning in place
        # before it could start frontier exploration. Keep the simple green marking
        # logic, but disable the active scan/approach behaviour by default.
        self.ENABLE_GREEN_SCAN = True
        ...
        # State machine: 'initial_scan', 'go_to_blue', 'search_yellow', 'explore', 'go_to_yellow', 'done'
        self.mission_state = 'initial_scan'

        # Do not cut the first 360-degree scan short when blue is detected.
        # The earlier version jumped to blue immediately, so the camera often never
        # rotated far enough to see the yellow pillar.
        self.COMPLETE_INITIAL_SCAN_BEFORE_BLUE = True
        self.POST_BLUE_YELLOW_SCAN_ENABLED = True
        
        # Camera field of view for coordinate estimation (radians)
        self.CAMERA_FOV_H = 1.047  # ~60 degrees, adjust if different

        # Pillars can be far away during the initial scan. With the old 5% global
        # threshold the robot repeatedly saw the blue pillar at ~2-3% coverage but
        # refused to save it, so it kept spinning/exploring instead of going to it.
        self.BLUE_RECORD_COVERAGE_THRESHOLD = 0.010   # 1.0% of ROI
        self.YELLOW_RECORD_COVERAGE_THRESHOLD = 0.010 # 1.0% of ROI
        ...
        self.DEBUG_DRAW_FRONTIERS = False
        self._current_frontiers = []  # Store current frontier cells for visualization
        self.PYGAME_SHOW_FRONTIERS = False
        self.MIN_FRONTIER_GOAL_DIST_CELLS = 20
        ...
        self.DEBUG_DRAW_GREEN_DETECTIONS = False
        ...
        # Visualization-only: do not let debug green detections permanently poison
        # the real occupancy grid. Real green blocking is handled by the green
        # obstacle functions below.
        self.GREEN_DETECTION_MARK_OCCUPIED = False
        ...
        self.PYGAME_UPDATE_EVERY_N_STEPS = 1
        self.PYGAME_CELL_SCALE = 1.5  # medium-size pygame map window (larger than 1x, smaller than 2x)
    ...
    def init_pygame_map(self):
        ...
        much more reliable inside Webots on Ubuntu/Wayland/XWayland, where the
        previous surface-based renderer could open a transparent/blank window.
        """
        ...
        try:
            pygame.init()
            pygame.font.init()
            scale = max(1.0, float(getattr(self, "PYGAME_CELL_SCALE", 1.0)))
            self._pygame_header_h = 54
            width = int(round(self.GRID_SIZE * scale))
            height = int(round(self.GRID_SIZE * scale)) + self._pygame_header_h

            # No RESIZABLE/SCALED/OPENGL flags here. A plain software window is
            # the most reliable option for Webots controller-side visualization.
            self._pygame_screen = pygame.display.set_mode((width, height), 0, 32)
            pygame.display.set_caption("RosBot live occupancy map")
            self._pygame_font = pygame.font.SysFont("Arial", 14)
            self._pygame_big_font = pygame.font.SysFont("Arial", 16, bold=True)
            self._pygame_enabled = True
            self._pygame_last_error_printed = False
            print(f"[pygame map] opened {width}x{height} live occupancy map window")
    ...
    def _make_pygame_map_surface(self):
        ...
        Colors:
            gray  = UNKNOWN
            white = FREE
            black = OCC / forced obstacle / green floor
        Overlays:
            purple  = current path
            red     = frontiers, if enabled
            cyan    = blue marker
        ...
        # Green-floor cells are stored as normal OCC, so they are drawn black.

        # Overlay frontiers in red only when explicitly enabled. Showing all
        # frontiers made the live map look noisy and hid the green obstacle patch.
        try:
            if getattr(self, "PYGAME_SHOW_FRONTIERS", False):
        ...
        # Overlay current path in purple.
        try:
            ...
            if path:
                ...
                if len(path_cells) >= 2:
                    pygame.draw.lines(surface, (170, 0, 255), False, path_cells, 2)
                for x, y in path_cells[::max(1, len(path_cells)//80)]:
                    surface.set_at((x, y), (200, 0, 255))
                if path_index < len(path):
                    self._draw_marker_on_surface(surface, path[path_index], (220, 0, 255), radius=3)
        except Exception:
            pass
    ...
    def _pygame_map_status_text(self):
        try:
            ...
            rx, ry, yaw, _ = self.get_pose()
            return f"free={free}  occ={occ}  unknown={unknown}  green_occ={len(getattr(self, 'green_obstacle_cells', set()))}  pose=({rx:.2f}, {ry:.2f}, {math.degrees(yaw):.0f} deg)"
        except Exception:
    ...
    def update_pygame_map(self, force=False):
        ...
        try:
            ...
            scale = max(1.0, float(getattr(self, "PYGAME_CELL_SCALE", 1.0)))
            header_h = int(getattr(self, "_pygame_header_h", 54))
            side = int(round(self.GRID_SIZE * scale))

            # Paint a real opaque background every frame. This prevents the
            # transparent-window effect seen with Webots on some desktops.
            self._pygame_screen.fill((20, 20, 20))

            map_surface = self._make_pygame_map_surface()
            if scale != 1:
                map_surface = pygame.transform.scale(map_surface, (side, side))
            self._pygame_screen.blit(map_surface, (0, header_h))
            pygame.draw.rect(self._pygame_screen, (220, 220, 220), pygame.Rect(0, header_h, side, side), 1)

            self._draw_pygame_label("RosBot live occupancy map", 6, 4, big=True)
            self._draw_pygame_label("gray=unknown | white=free | black=obstacle/green floor | purple=path", 6, 25)
            self._draw_pygame_label(self._pygame_map_status_text(), 6, 39)
    ...
    def process_camera(self):
        ...
        # Keep green detections even while a path is active. The green-scan
        # behaviour is controlled by the mission-state logic below, but the
        # occupancy map still needs the green detection so the planner can avoid it.
    ...
    def reset_360_scan(self):
        """Reset the yaw accumulator used for an in-place 360-degree camera scan."""
        self.initial_scan_start_yaw = None
        self.initial_scan_last_yaw = None
        self.initial_scan_accumulated = 0.0

    def perform_initial_scan(self, label="initial"):
        """Perform an in-place 360-degree camera scan. Returns True when complete."""
        ...
        if self.initial_scan_start_yaw is None:
            ...
            self.initial_scan_accumulated = 0.0
            print(f"Starting 360-degree {label} scan...")
            return False
        ...
        if self.initial_scan_accumulated >= 2.0 * math.pi:
            print(f"360-degree {label} scan complete! Accumulated: {math.degrees(self.initial_scan_accumulated):.1f} degrees")
    ...
    def handle_color_detection(self, detection_info):
        ...
        # Color-specific thresholds. Blue often appears smaller/farther during
        # the initial scan, so using a single 5% threshold misses it.
        BLUE_RECORD_COVERAGE_THRESHOLD = float(getattr(self, 'BLUE_RECORD_COVERAGE_THRESHOLD', 0.010))
        YELLOW_RECORD_COVERAGE_THRESHOLD = float(getattr(self, 'YELLOW_RECORD_COVERAGE_THRESHOLD', 0.020))
        ...
        if color == 'blue' and not self.blue_found:
            ...
            if coverage >= BLUE_RECORD_COVERAGE_THRESHOLD:
                ...
                print(f"BLUE object detected but coverage too low ({coverage*100.0:.1f}% < {BLUE_RECORD_COVERAGE_THRESHOLD*100.0:.1f}%), waiting for better view...")
            ...
            if coverage >= YELLOW_RECORD_COVERAGE_THRESHOLD:
                ...
                print(f"YELLOW object detected but coverage too low ({coverage*100.0:.1f}% < {YELLOW_RECORD_COVERAGE_THRESHOLD*100.0:.1f}%), waiting for better view...")
    ...
    def _green_detection_bottom_info(self, green_det):
        """Return True when green is in the lower part of the camera ROI.

        This is the same basic trigger used by the attached reference controller:
        when the green centroid is in the bottom 30% of the ROI, the green field
        is treated as close and a forward obstacle zone is marked.
        """
        if green_det is None:
            return False
        try:
            _, cy = green_det['centroid_px']
            img_w = int(green_det['img_width'])
            img_h = int(green_det['img_height'])
            ...
            return float(cy) >= float(bottom_threshold_y)
        except Exception:
            return False

    def _green_detection_bearing(self, green_det):
        """Return world-frame bearing toward the visible green blob."""
        try:
            cx, _ = green_det['centroid_px']
            img_w = float(green_det['img_width'])
            img_h = float(green_det['img_height'])
            _, _, ryaw, _ = self.get_pose()
            cx_normalized = (float(cx) - img_w / 2.0) / max(1.0, img_w / 2.0)
            fov_h = self._camera_fov_h(int(img_w), int(img_h))
            angle_offset = -cx_normalized * (fov_h / 2.0)
            return ryaw + angle_offset
        except Exception:
            _, _, ryaw, _ = self.get_pose()
            return ryaw

    def force_green_cell(self, i, j):
        """Permanently mark a map cell as green-floor obstacle.

        Green is stored as a normal occupied cell, so pygame shows it black like
        other blocked cells. Keeping a separate set prevents lidar free-space
        updates from clearing it later.
        """
        if not self.inside_map(i, j):
            return False
        cell = (int(i), int(j))
        already = cell in getattr(self, 'green_obstacle_cells', set())
        if not hasattr(self, 'green_obstacle_cells'):
            self.green_obstacle_cells = set()
        self.green_obstacle_cells.add(cell)
        self.force_occupy_cell(int(i), int(j))
        # Green changes the planning map; invalidate inflated-obstacle cache.
        self._hard_blocked_cache = None
        self._hard_blocked_cache_scan_id = None
        return not already

    def _invalidate_path_if_green_blocked(self, blocked_cells):
        if not blocked_cells:
            return
        blocked = set(blocked_cells)
        if self.current_goal in blocked:
            self.current_goal = None
            self.need_new_goal = True
        if self.path:
            remaining = self.path[max(0, int(self.path_index)):] if self.path_index is not None else self.path
            if any(c in blocked for c in remaining):
                print("[GreenMap] Current path crosses green floor - clearing path for replan.")
                self.path = None
                self.path_goal = None
                self.path_index = 0
                self.need_new_goal = True

    def mark_green_obstacle_zone(self, detection_info, radius_m=None, source="camera"):
        """Compatibility entry point used by older code.

        The last compact version called this every frame and created an unwanted
        wedge. It now delegates to the reference-style green-floor obstacle logic
        only when explicitly called.
        """
        return self.mark_green_floor_obstacles(detection_info)

    def mark_green_floor_obstacles(self, detection_info):
        """Reference-style green-floor mapping adapted for Supervisor-free code.

        Logic adapted from the attached controller:
        - ignore tiny green noise,
        - if green is low in the ROI, mark a forward forbidden cone,
        - otherwise mark a compact distant patch at the estimated green location,
        - clear/replan if the current path crosses the newly blocked cells.

        Supervisor z-height is not available in this controller, so the elevated
        branch is kept logically disabled unless a real z estimate is provided.
        """
        green_det = self._get_detection_for_color(detection_info, 'green')
        if green_det is None:
            return False
        ...
        try:
            ...
            if now < float(getattr(self, '_green_cooldown_until_time', 0.0)):
                return False
        except Exception:
            now = None
        ...
        coverage = float(green_det.get('coverage', 0.0))

        # Same threshold as the attached controller: 1.5% of ROI.
        if coverage < 0.015:
            return False

        green_in_bottom_roi = self._green_detection_bottom_info(green_det)

        rx, ry, ryaw, rz = self.get_pose()
        ri, rj = self.world_to_grid(rx, ry)

        # Supervisor-free odometry has no real absolute z; this stays false in
        # normal use. If z_est is later provided by a real sensor, this branch works.
        ...
        # Use robot-position key, like the reference code, to avoid repeated
        # marking of the same local patch.
        ...
        def add_blocked_cell(gi, gj):
            if not self.inside_map(gi, gj):
                return
            cell = (int(gi), int(gj))
            if cell in blocked_set:
                return
            blocked_set.add(cell)
            self.force_green_cell(cell[0], cell[1])
            blocked_cells.append(cell)

        if green_in_bottom_roi and elevated:
            # Reference-code elevated-platform branch. This is normally inactive
            # without Supervisor z, but kept for compatibility with a real z sensor.
            print(f"GREEN PLATFORM DETECTED - Robot elevated {z_diff:.4f}m! Blocking ahead and sides!")
            ...
            for forward_dist in range(0, forward_cells):
                ...
                for side_offset in range(-side_cells, side_cells + 1):
                    ...
                    add_blocked_cell(gi, gj)
            print(f"Marked {len(blocked_cells)} cells as elevated green platform obstacles")

        elif green_in_bottom_roi:
            # Attached-code behavior: green is close, block the forward green danger zone.
            # Slightly reduced from 0.75m/45deg to avoid painting the huge black fan
            # that appeared in the previous iteration, while preserving the reference logic.
            print("GREEN IN BOTTOM ROI - Marking close green floor ahead as blocked!")
            self.marked_green_regions.add(region_key)
            front_distance_m = float(getattr(self, 'GREEN_REFERENCE_FRONT_DISTANCE_M', 0.55))
            cone_half_angle = math.radians(float(getattr(self, 'GREEN_REFERENCE_CONE_DEG', 32.0)))
            max_dist_cells = int(front_distance_m / self.MAP_RES)
            robot_clear_cells = max(1, int(0.16 / self.MAP_RES))
            clear2 = robot_clear_cells * robot_clear_cells
            for dist_step in range(1, max_dist_cells + 1):
                dist_world = dist_step * self.MAP_RES
                num_angle_samples = max(3, int(dist_step / 5))
                for angle_idx in range(num_angle_samples):
                    ...
                    gi, gj = self.world_to_grid(sample_x, sample_y)
                    if (gi - ri) * (gi - ri) + (gj - rj) * (gj - rj) <= clear2:
                        continue
                    add_blocked_cell(gi, gj)
            print(f"Marked {len(blocked_cells)} cells as close green floor obstacles ahead of robot")

        else:
            # Attached-code behavior: green is further away, mark a compact patch
            # at the estimated depth/bearing. This fixes the issue where far green
            # was visible but not entered into the map at all.
            depth = self._get_depth_at(cx, cy)
            if depth is None or not math.isfinite(float(depth)):
                depth = 1.5
            # Cap far projection to avoid false green behind the real patch.
            depth = max(0.20, min(float(depth), 1.6))

            cx_normalized = (cx - img_w / 2.0) / (img_w / 2.0)
            angle_offset = -cx_normalized * (self._camera_fov_h(img_w, img_h) / 2.0)
            obj_angle = ryaw + angle_offset
            ...
            self.marked_green_regions.add(region_key)
            radius_cells = int(getattr(self, 'GREEN_REFERENCE_DISTANT_RADIUS_CELLS', 6))
            for di in range(-radius_cells, radius_cells + 1):
                for dj in range(-radius_cells, radius_cells + 1):
                    if di * di + dj * dj > radius_cells * radius_cells:
                        continue
                    add_blocked_cell(center_i + di, center_j + dj)
            print(f"Marked {len(blocked_cells)} cells as distant green floor obstacle patch at ({center_i}, {center_j})")

        if blocked_cells:
            self._invalidate_path_if_green_blocked(blocked_cells)
            if hasattr(self, '_current_frontiers'):
                invalidated_count = 0
                blocked_lookup = set(blocked_cells)
                for (fi, fj) in self._current_frontiers:
                    if (fi, fj) in blocked_lookup:
                        self.mark_goal_visited((fi, fj))
                        invalidated_count += 1
                if invalidated_count > 0:
                    print(f"Invalidated {invalidated_count} frontiers in green danger zone")
            try:
                # Shorter than the source 12s cooldown for floor marking; the
                # region key still prevents rapid repeated marking at same pose.
                self._green_cooldown_until_time = float(self.robot.getTime()) + float(getattr(self, 'GREEN_MARK_COOLDOWN_S', 1.2))
            except Exception:
                pass
            return True
        return False
    ...
    def mark_green_poison_zone(self, detection_info, radius_m=1.0):
        """Reference-style poison marking while navigating, bounded for safety.

        The attached controller used this during go_to_blue/go_to_yellow to force
        replanning away from green. Here it is adapted to be persistent but capped
        so it does not paint a huge false black circle.
        """
        green_det = self._get_detection_for_color(detection_info, 'green')
        if green_det is None:
            return False
        ...
        # Cap the poison radius to avoid the large false circles seen before.
        radius_m = min(float(radius_m), float(getattr(self, 'GREEN_REFERENCE_POISON_RADIUS_M', 0.45)))
        r_cells = int(max(1, radius_m / float(self.MAP_RES)))
        r2 = r_cells * r_cells
        blocked = set()
        for dj in range(-r_cells, r_cells + 1):
            for di in range(-r_cells, r_cells + 1):
                ...
                if self.inside_map(gi, gj):
                    self.force_green_cell(gi, gj)
                    blocked.add((gi, gj))

        if blocked:
            print(f"[GreenPoison] Marked {len(blocked)} green obstacle cells near ({center_i}, {center_j})")
            self._invalidate_path_if_green_blocked(blocked)
            return True
        return False
    ...
    def _nearest_free_cell_to(self, cell, max_radius=12, require_not_hard_blocked=False):
        """Return a nearby FREE cell around cell.

        Supervisor-free odometry and green/floating-wall marks can occasionally put
        the robot's estimated cell on UNKNOWN/OCC or inside inflated space. If BFS
        starts from such a cell, exploration never finds a reachable frontier and
        the robot just spins. This helper snaps the start back into the known free
        connected area.
        """
        if cell is None:
            return None
        ci, cj = int(cell[0]), int(cell[1])
        hard_blocked = self._compute_hard_blocked() if require_not_hard_blocked else None
        for r in range(0, int(max_radius) + 1):
            for dj in range(-r, r + 1):
                for di in range(-r, r + 1):
                    ...
                    if self.display_state[nj][ni] != self.FREE:
                        continue
                    if hard_blocked is not None and hard_blocked[nj][ni]:
                        continue
                    return (ni, nj)
        return None

    def _nearest_free_frontier_fallback(self, frontiers, start_cell):
        """Fallback frontier selector that does not require hard-inflated BFS.

        This is intentionally conservative: it still requires the target frontier
        cell to be FREE, but it avoids the failure mode where every reachable
        frontier is rejected only because inflation is too strict for the freshly
        built Supervisor-free map.
        """
        if not frontiers:
            return None
        si, sj = int(start_cell[0]), int(start_cell[1])
        candidates = []
        near_candidates = []
        min_d2 = int(getattr(self, 'MIN_FRONTIER_GOAL_DIST_CELLS', 20)) ** 2
        for i, j in frontiers:
            ...
            if self.display_state[j][i] != self.FREE:
                continue
            if self.is_goal_visited((i, j)):
                continue
            d2 = (i - si) * (i - si) + (j - sj) * (j - sj)
            if d2 >= min_d2:
                candidates.append((d2, (i, j)))
            else:
                near_candidates.append((d2, (i, j)))
        if not candidates and not near_candidates:
            return None
        use = candidates if candidates else near_candidates
        use.sort(key=lambda x: x[0])
        return use[0][1]

    def find_nearest_reachable_frontier(self, frontiers, start_cell):
        """Return the nearest reachable frontier.

        First try a normal BFS through FREE cells while avoiding hard-inflated
        obstacles. If that rejects everything, retry without the hard-inflation
        restriction. This avoids the common Supervisor-free failure where the map
        is correct enough for exploration but the inflation buffer disconnects the
        start cell from all frontier cells, causing endless spinning.
        """
        if not frontiers:
            return None

        start = self._nearest_free_cell_to(start_cell, max_radius=10, require_not_hard_blocked=False)
        if start is None:
            print(f"[Frontier] start cell {start_cell} is not on/near FREE space")
            return None
        ...
        for (i, j) in frontiers:
            ii, jj = int(i), int(j)
            if (ii - start[0]) * (ii - start[0]) + (jj - start[1]) * (jj - start[1]) >= min_d2:
                far_frontiers.append((ii, jj))
        if far_frontiers:
            frontier_set = set(far_frontiers)
        else:
            frontier_set = set((int(i), int(j)) for (i, j) in frontiers)
        neighbors = ((1, 0), (-1, 0), (0, 1), (0, -1))

        def bfs(use_hard_blocked):
            hard_blocked = self._compute_hard_blocked() if use_hard_blocked else None
            ...
            while q:
                ...
                if (i, j) in frontier_set and self.display_state[j][i] == self.FREE:
                    if not self.is_goal_visited((i, j)):
                        if hard_blocked is None or not hard_blocked[j][i]:
                            return (i, j)
        ...
        chosen = bfs(use_hard_blocked=True)
        if chosen is not None:
            return chosen

        chosen = bfs(use_hard_blocked=False)
        if chosen is not None:
            print(f"[Frontier] Using relaxed reachable frontier {chosen} from start {start}")
            return chosen

        chosen = self._nearest_free_frontier_fallback(frontiers, start)
        if chosen is not None:
            print(f"[Frontier] Using nearest-FREE fallback frontier {chosen} from start {start}")
        return chosen
    ...
    def _line_is_clear(self, start_cell, end_cell):
        ...
        while True:
            ...
            if self.display_state[y][x] != self.FREE:
                return False
            # For waypoint lookahead, hard inflation can be too strict in narrow
            # corridors. Occupancy is still checked above, so do not reject only
            # because the cell lies in the inflation buffer.
            # if hard_blocked[y][x]:
            #     return False
    ...
    def astar(self, start, goal):
        """A* pathfinding from start=(i,j) to goal=(i,j).

        The first pass uses the normal hard/soft inflation. If that fails, a
        second relaxed pass removes inflation but still requires FREE cells. This
        keeps the robot moving in narrow fresh maps instead of spinning forever
        when inflation temporarily disconnects all frontiers.
        """
        start = self._nearest_free_cell_to(start, max_radius=10, require_not_hard_blocked=False) or start
        goal = self._nearest_free_cell_to(goal, max_radius=8, require_not_hard_blocked=False) or goal
        start = (int(start[0]), int(start[1]))
        goal = (int(goal[0]), int(goal[1]))

        if not self.inside_map(start[0], start[1]) or not self.inside_map(goal[0], goal[1]):
            return None
        if self.display_state[start[1]][start[0]] != self.FREE:
            return None
        if self.display_state[goal[1]][goal[0]] != self.FREE:
            return None

        def heuristic(a, b):
            return abs(b[0] - a[0]) + abs(b[1] - a[1])

        def run_search(r_hard, r_soft, soft_weight):
            open_set = []
            ...
            while open_set:
                ...
                for di, dj in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                    ...
                    if min_d2 is not None:
                        ...
                        if r_hard > 0 and min_d <= r_hard:
                            # Do not block the exact start/goal when they were
                            # already snapped to FREE; otherwise planning can
                            # fail while the robot is beside a wall.
                            if (ni, nj) != start and (ni, nj) != goal:
                                continue
                        if r_soft > r_hard and min_d < r_soft:
                            t = (r_soft - min_d) / max(1.0, (r_soft - r_hard))
                            soft_penalty = soft_weight * max(0.0, min(1.0, t))

                    neighbor = (ni, nj)
                    if neighbor in visited:
                        continue
                    new_g = g + 1.0 + soft_penalty
                    heapq.heappush(open_set, (new_g + heuristic(neighbor, goal), new_g, neighbor, path + [neighbor]))
            return None

        path = run_search(
            int(getattr(self, 'HARD_INFLATION_RADIUS_CELLS', 0)),
            int(max(getattr(self, 'SOFT_INFLATION_RADIUS_CELLS', 0), getattr(self, 'HARD_INFLATION_RADIUS_CELLS', 0))),
            float(getattr(self, 'SOFT_INFLATION_WEIGHT', 0.0)),
        )
        if path:
            return path

        path = run_search(0, 0, 0.0)
        if path:
            print(f"[A*] relaxed path used start={start} goal={goal} len={len(path)}")
        return path
...
if __name__ == "__main__":
    ...
    try:
        while controller.step() != -1:
            ...
            # Green mapping/avoidance is handled below using the reference-style
            # green scan/floor/poison logic, adapted for Supervisor-free pose.

            # Record pillar detections passively even while driving to a goal.
            # The old code only handled yellow in explore/initial_scan, so short
            # yellow glimpses while going to blue were lost.
            if controller.mission_state in ('go_to_blue', 'go_to_yellow'):
                controller.handle_color_detection(detection_info)

            # Debug green overlay is disabled by default; real green is black OCC.
            if getattr(controller, 'DEBUG_DRAW_GREEN_DETECTIONS', False):
            ...
            if (getattr(controller, 'ENABLE_GREEN_SCAN', False) and not green_cooldown_active) and controller.mission_state not in ('initial_scan', 'search_yellow', 'go_to_blue', 'go_to_yellow') and detection_info:
            ...
            if (not green_cooldown_active) and controller.mission_state not in ('initial_scan', 'search_yellow', 'go_to_blue', 'go_to_yellow'):
            ...
            if controller.mission_state == 'initial_scan':
                ...
                # Do not normally interrupt the first 360 scan when blue is detected.
                # Finishing the scan gives the camera a chance to see yellow too.
                if action == 'go_to_blue' and not getattr(controller, 'COMPLETE_INITIAL_SCAN_BEFORE_BLUE', True):
                    controller.mission_state = 'go_to_blue'
                    controller.stop()
                ...
                scan_complete = controller.perform_initial_scan(label="initial pillar-search")
            ...
            if controller.mission_state == 'go_to_blue':
                ...
                if controller.reached_color_object(controller.blue_coords, threshold_m=0.35):
                    ...
                    # If only blue found, decide next state based on yellow
                        ...
                        if getattr(controller, 'POST_BLUE_YELLOW_SCAN_ENABLED', True):
                            controller.mission_state = 'search_yellow'
                            controller.reset_360_scan()
                            print("Yellow not yet found. Performing 360-degree yellow search before exploration.")
                        else:
                            controller.mission_state = 'explore'
                ...
                if controller.current_goal != blue_goal or not controller.path:
                    ...
                    if blue_path:
                        ...
                        controller.blue_plan_failure_count = 0
                        print(f"Setting goal to BLUE at grid {blue_goal}, path_len={len(blue_path)}")
                    else:
                        # No path yet. Do not stay stopped forever; return to
                        # frontier exploration so the robot can map a route to blue.
                        controller.blue_plan_failure_count += 1
                        print(f"No path to BLUE goal {blue_goal} yet (failure {controller.blue_plan_failure_count})")
                        controller.current_goal = None
                        controller.path = None
                        controller.path_goal = None
                        controller.path_index = 0
                        if controller.blue_plan_failure_count >= 3:
                            print("No path to BLUE after repeated attempts - exploring to open the map.")
                            controller.mission_state = 'explore'
                            controller.need_new_goal = True
                            controller.blue_plan_failure_count = 0
                        continue
            ...
            # --- STATE: SEARCH YELLOW ---
            if controller.mission_state == 'search_yellow':
                # After reaching blue, perform one controlled 360-degree camera scan.
                # This catches yellow immediately if it is visible from the blue area,
                # instead of jumping straight into frontier exploration.
                controller.handle_color_detection(detection_info)

                if controller.yellow_found:
                    controller.mission_state = 'go_to_yellow'
                    controller.current_goal = None
                    controller.need_new_goal = True
                    controller.path = None
                    ...
                    controller.path_index = 0
                    controller.stop()
                    print("Yellow found during post-blue 360 scan. Navigating to YELLOW.")
                    continue

                scan_complete = controller.perform_initial_scan(label="post-blue yellow-search")
                if scan_complete:
                    controller.reset_360_scan()
                    if controller.yellow_found:
                        controller.mission_state = 'go_to_yellow'
                        print("Post-blue scan complete. Yellow found; navigating to YELLOW.")
                    else:
                        controller.mission_state = 'explore'
                        print("Post-blue scan complete. Yellow not found; starting exploration.")
                    controller.current_goal = None
                    controller.need_new_goal = True
                    controller.path = None
                    controller.path_goal = None
            ...
            if controller.mission_state == 'go_to_yellow':
                ...
                if controller.current_goal != yellow_goal or not controller.path:
                    ...
                    if direct_path:
                        ...
                        if controller.path and controller.path[0] == (ri, rj):
                            controller.path_index = 1
                        controller.yellow_plan_failure_count = 0
                        ...
                        controller.yellow_plan_failure_count += 1
                        print(f"No path to yellow goal {yellow_goal} - blacklisting and trying another nearby cell (failure {controller.yellow_plan_failure_count})")
