...
class RosbotExplorer:
    def __init__(self):
        ...
        # 1.1m: 0.8 detected too late (robot committed to the wall), but 1.5
        # pulled in distant FLOOR pixels whose small height errors got them
        # wrongly marked, blacking out free space. 1.1 detects with room to
        # replan while keeping the floor-error problem small; the per-cell
        # minimum-point-count filter (LOWWALL_MIN_POINTS_PER_CELL) handles the
        # residual scattered noise.
        self.DEPTH_OBSTACLE_MAX_DIST = 1.10
        self.LOWWALL_MIN_POINTS_PER_CELL = 4
        self.FLOATING_WALL_MIN_HEIGHT = 0.05  # below = floor reflections, ignore
        # The robot's real physical top height. A depth hit whose LOWEST point at a
        # cell is below this = the robot body would collide → obstacle; at/above =
        # clears under → passable. This IS the "match the height" rule the whole
        # feature turns on, so it must equal the robot's true height. Kept at 0.22
        # (RosBot chassis + sensor mast). If the console diagnostic below shows a
        # too-low wall's minH sitting just ABOVE this, raise it; if a passable wall
        # is wrongly marked, lower it.
        self.ROBOT_CLEARANCE_HEIGHT = 0.22
        self.FLOATING_WALL_CAMERA_HEIGHT_M = 0.17
        # Print a throttled [LowWall] line whenever hits are in view, so the height
        # math can be checked against reality from the console instead of guessed.
        self.LOWWALL_DEBUG = True
    ...
    def update_pygame_map(self, force=False):
        ...
        try:
            ...
            self._draw_pygame_label("gray=unknown | white=free | black=obstacle | magenta=robot | green=path", 6, 25)
    ...
    def debug_record_green_detection(self, detection_info, radius_m=0.10):
        ...
        # NEAR-DEDUP SMEAR GUARD. Without this, a permanent disk is splatted on
        # EVERY frame green is in view, and the jittered per-frame position
        # estimates creep the blob outward sighting after sighting until it
        # swallows the normal floor beside the patch (the "more green marked than
        # actually green" symptom). Skip if this disk's center lands within one
        # disk radius of any previously marked green cell: repeat views of the
        # same spot are absorbed, while genuinely new parts of the patch (more
        # than a radius from all existing marks, as the robot moves along it)
        # still get their own disks and get covered.
        if not hasattr(self, '_green_marked_cells'):
            self._green_marked_cells = set()
        if self.inside_map(ci, cj):
            for _dj in range(-r_cells, r_cells + 1):
                for _di in range(-r_cells, r_cells + 1):
                    if _di * _di + _dj * _dj > r2:
                        continue
                    if (ci + _di, cj + _dj) in self._green_marked_cells:
                        return
        ...
        if r_cells <= 0:
            if self.inside_map(ci, cj):
                ...
                if getattr(self, 'GREEN_DETECTION_MARK_OCCUPIED', False):
                    ...
                    self._green_marked_cells.add((ci, cj))
            return
            ...
            for di in range(-r_cells, r_cells + 1):
                ...
                if self.inside_map(ni, nj):
                    ...
                    if getattr(self, 'GREEN_DETECTION_MARK_OCCUPIED', False):
                        ...
                        self._green_marked_cells.add((ni, nj))
    ...
    def _get_depth_from_color_mask(self, mask, x0, y0, percentile=35.0):
        """Estimate object depth from the pixels that actually matched the color.

        Depth at the mask CENTROID fails when the object is visually split by an
        occluder — e.g. the blue pillar seen through a gap between wall bars: the
        centroid of two split blobs lands ON the occluder between them, so the
        centroid-pixel depth reads the occluder's distance, and the pillar gets
        locked at the WALL's position (observed directly in testing). Sampling
        depth at the MASK's own pixels is immune: every sampled pixel is on the
        object. A low-ish percentile biases toward the object's near surface
        while rejecting stray background pixels at the blob edges.
        """
        if mask is None or not self.camera_depth:
            return None
        ys, xs = np.where(mask)
        if len(xs) == 0:
            return None
        try:
            ...
            ranges = self.camera_depth.getRangeImage()
            if not ranges or dw <= 0 or dh <= 0:
                return None
            depth_img = np.asarray(ranges, dtype=np.float32).reshape((dh, dw))
        except Exception:
            return None

        rgb_w = int(self.camera_rgb.getWidth()) if self.camera_rgb else int(mask.shape[1])
        rgb_h = int(self.camera_rgb.getHeight()) if self.camera_rgb else int(mask.shape[0])

        dx = np.clip(np.rint((x0 + xs.astype(np.float32)) * (dw / float(max(1, rgb_w)))).astype(np.int32), 0, dw - 1)
        dy = np.clip(np.rint((y0 + ys.astype(np.float32)) * (dh / float(max(1, rgb_h)))).astype(np.int32), 0, dh - 1)
        values = depth_img[dy, dx]
        rmin = float(self.camera_depth.getMinRange())
        rmax = float(self.camera_depth.getMaxRange())
        valid = np.isfinite(values) & (values > rmin) & (values < rmax)
        if not np.any(valid):
            return None
        return float(np.percentile(values[valid], max(0.0, min(100.0, percentile))))
    ...
    def process_camera(self):
        ...
        if green_mask is not None:
            ...
            coverage["green"] = float(g_count) / float(max(1, green_mask.size))
            if g_count > 0:
        ...
        if coverage:
            for name, frac in sorted(coverage.items(), key=lambda kv: kv[1], reverse=True):
                ...
                # Blue: depth from the mask's OWN pixels (occluder-immune). The
                # centroid pixel can land on a wall bar in front of a visually
                # split pillar and read the wall's distance instead — locking blue
                # at the wall. See _get_depth_from_color_mask().
                if name == 'blue':
                    _dm = self._get_depth_from_color_mask(color_masks.get('blue'), x0, y0)
                    if _dm is not None:
                        det['depth_m'] = _dm
    ...
    def handle_color_detection(self, detection_info):
        ...
        if color == 'blue' and not self.blue_found:
            ...
            if coverage >= PILLAR_RECORD_COVERAGE_THRESHOLD:
                ...
                # Prefer the mask-pixel depth attached by process_camera — it is
                # immune to the split-blob/occluder failure where the centroid
                # pixel lands on a wall bar IN FRONT of the pillar and reads the
                # wall's distance. Centroid-pixel depth is only a fallback.
                d_b = detection_info.get('depth_m')
                if d_b is None:
                    cx_b, cy_b = detection_info['centroid_px']
                    d_b = self._get_depth_at(cx_b, cy_b, window_px=0)
                if d_b is None:
                    print("BLUE seen but no depth reading — waiting for a better view...")
                    return 'continue'
                if d_b > 2.5:
    ...
    def detect_and_mark_low_walls(self):
        ...
        # Per-cell lowest hit height (passability) AND hit count (real-object
        # test). A solid low wall returns MANY depth pixels clustered on one grid
        # cell; a floor pixel whose height is slightly mis-computed (camera pitch/
        # calibration error, worse at longer range) is sparse and scattered — one
        # or two stray hits per cell. Requiring a minimum hit count per cell keeps
        # the wall while rejecting the scattered floor noise that was blacking out
        # free space when the range was extended.
        cell_min_h = {}
        cell_count = {}
        for i_pt in range(len(Xw)):
            ...
            if not self.inside_map(gi, gj):
                continue
            cell_count[(gi, gj)] = cell_count.get((gi, gj), 0) + 1
            if (gi, gj) not in cell_min_h or wh < cell_min_h[(gi, gj)]:
                cell_min_h[(gi, gj)] = wh
        # Drop sparse cells (scattered floor-noise pixels, not a solid wall).
        min_pts = int(getattr(self, 'LOWWALL_MIN_POINTS_PER_CELL', 4))
        cell_min_h = {c: h for c, h in cell_min_h.items() if cell_count.get(c, 0) >= min_pts}

        # Diagnostic: report what heights are actually being measured so the
        # "match the height" behavior can be verified from the console — face the
        # problem wall and read minH. If a too-low wall shows minH just ABOVE
        # ROBOT_CLEARANCE_HEIGHT it's being wrongly judged passable (raise the
        # threshold); if minH is far off from the real slab height the camera-
        # height/geometry is off.
        if getattr(self, 'LOWWALL_DEBUG', False) and cell_min_h:
            self._lowwall_diag = getattr(self, '_lowwall_diag', 0) + 1
            if self._lowwall_diag % 30 == 0:
                _hs = list(cell_min_h.values())
                _band = sum(1 for _h in _hs if self.FLOATING_WALL_MIN_HEIGHT < _h < self.ROBOT_CLEARANCE_HEIGHT)
                print(f"[LowWall] cells={len(_hs)} minH={min(_hs):.3f} maxH={max(_hs):.3f} "
                      f"in_body_band(<{self.ROBOT_CLEARANCE_HEIGHT:.2f})={_band}")

        # Light temporal confirmation: a level-camera assumption means chassis
        # PITCH (braking/accel) can momentarily throw floor pixels into the body
        # band. Require 2 sightings before locking so a single rocked frame can't
        # create a phantom, but detection still fires within ~2 poll cycles — fast
        # enough at the 1.5m range to react. Real walls re-confirm trivially.
        if not hasattr(self, '_lowwall_hits'):
            self._lowwall_hits = {}
        seen_now = set()
        for (gi, gj), min_h in cell_min_h.items():
            if min_h >= self.ROBOT_CLEARANCE_HEIGHT:
                continue  # entirely above the robot — passable, leave free
            if self.display_state[gj][gi] == self.OCC:
                continue  # already an obstacle (lidar or previous pass)
            seen_now.add((gi, gj))

        newly_locked = 0
        for cell in seen_now:
            self._lowwall_hits[cell] = self._lowwall_hits.get(cell, 0) + 1
            if self._lowwall_hits[cell] >= 2:
                self.force_occupy_cell(cell[0], cell[1])
                del self._lowwall_hits[cell]
                newly_locked += 1
        for cell in list(self._lowwall_hits.keys()):
            if cell not in seen_now:
                self._lowwall_hits[cell] -= 1
                if self._lowwall_hits[cell] <= 0:
                    del self._lowwall_hits[cell]
            ...
            if controller.mission_state == 'go_to_blue':
                ...
                if (controller.current_goal != blue_goal or not controller.path) and (
                    ...
                    if blue_path:
                        ...
                        controller.HARD_INFLATION_RADIUS_CELLS = 2
                        ...
                        controller._move_to_waypoint(blue_goal[0], blue_goal[1])
                        continue
                    ...
                    if chosen is None:
                        ...
                        if controller.frontier_failure_count >= 6:
                            ...
                            if escape_path:
                                ...
                                controller.path_index = 0
                                controller.frontier_failure_count = 0
                                controller._map_trap_bubble_count = 0
                                continue

                            # SELF-EVIDENCE RECOVERY: even minimal inflation found
                            # nothing — the robot is MAP-TRAPPED: OCC cells (often
                            # phantom marks) sit on/around the very cells it is
                            # standing on, so the reachability search dies at its
                            # first step regardless of inflation. But the robot's
                            # physical presence PROVES its own footprint is free.
                            # Clear a small bubble of the map around the actual
                            # pose (including forced cells) so the search can step
                            # off — the surrounding real walls are untouched and
                            # get re-observed by lidar immediately anyway.
                            controller._map_trap_bubble_count = getattr(controller, '_map_trap_bubble_count', 0) + 1
                            if controller._map_trap_bubble_count >= 3:
                                controller._map_trap_bubble_count = 0
                                rx_bt, ry_bt, _, _ = controller.get_pose()
                                ri_bt, rj_bt = controller.world_to_grid(rx_bt, ry_bt)
                                r_bub = int(0.30 / controller.MAP_RES)
                                cleared_bt = 0
                                for dj_bt in range(-r_bub, r_bub + 1):
                                    for di_bt in range(-r_bub, r_bub + 1):
                                        if di_bt * di_bt + dj_bt * dj_bt > r_bub * r_bub:
                                            continue
                                        ni_bt, nj_bt = ri_bt + di_bt, rj_bt + dj_bt
                                        if not controller.inside_map(ni_bt, nj_bt):
                                            continue
                                        if controller.display_state[nj_bt][ni_bt] == controller.OCC:
                                            controller.forced_occupied_cells.discard((ni_bt, nj_bt))
                                            controller.grid[nj_bt][ni_bt] = controller.L_PRIOR
                                            controller.confirm_counters[nj_bt][ni_bt] = -1
                                            controller.display_state[nj_bt][ni_bt] = controller.FREE
                                            controller.draw_pixel(ni_bt, nj_bt, controller.FREE)
                                            cleared_bt += 1
                                if cleared_bt:
                                    controller._hard_blocked_cache = None
                                    controller._hard_blocked_cache_scan_id = None
                                    print(f"[MapTrap] Robot's own position proves {cleared_bt} OCC cells "
                                          f"around it are phantom — cleared 0.3m bubble, replanning")
            ...
            if controller.mission_state == 'go_to_yellow':
                ...
                if (controller.current_goal != yellow_goal or not controller.path) and (
                    ...
                    if direct_path:
                        ...
                        controller._move_to_waypoint(yellow_goal[0], yellow_goal[1])
                        continue
